...
{
    uint16_t ccr = duty_ratio * CCR0_MAX / 100;
    if(dir == 1)
    {
        pwm_duty(0, num, 0);
        pwm_duty(1, num, ccr);
...
void motor_stop()
{
    pwm_duty(0, 1, CCR0_MAX);
    pwm_duty(1, 1, CCR0_MAX);
    pwm_duty(0, 2, CCR0_MAX);
    pwm_duty(1, 2, CCR0_MAX);
    pwm_duty(0, 3, CCR0_MAX);
    pwm_duty(1, 3, CCR0_MAX);
    pwm_duty(0, 4, CCR0_MAX);
    pwm_duty(1, 4, CCR0_MAX);
...
{
    angle += 135;
    angle = Limiter(angle, 50, 220);
    float duty_ratio = angle / 27 + 2.5;
